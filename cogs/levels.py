...
import math
import config
from utils.embeds import success_embed, error_embed, info_embed, GuildEmbed, send_ephemeral
...
class Levels(commands.Cog):
    ...
    async def _level_up(self, member, level, guild_id):
        g = await self.bot.db.get_guild(guild_id)
        lang = await self.bot.get_lang(guild_id)
        msg = g.get("level_message", self.bot.t(lang, "levels.level_up_msg", user=member.mention, level=level, name=member.name)).format(user=member.mention, level=level, name=member.name)
        ch_id = g.get("level_channel")
        embed = GuildEmbed(title=self.bot.t(lang, "levels.level_up"), description=msg, color=config.COLORS["purple"], guild=member.guild)
    ...
    async def rank(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        embed = GuildEmbed(title=self.bot.t(lang, "levels.stats_title", user=user.display_name), color=user.color or config.EMBED_COLOR)
        embed.set_thumbnail(url=user.display_avatar.url)
        embed.add_field(name=self.bot.t(lang, "levels.level"), value=str(level), inline=True)
        embed.add_field(name=self.bot.t(lang, "levels.xp"), value=self.bot.t(lang, "levels.needed_xp", xp=xp, needed=needed), inline=True)
        embed.add_field(name=self.bot.t(lang, "levels.total_xp"), value=f"{total:,}f", inline=True)
        embed.add_field(name=self.bot.t(lang, "levels.progress"), value=f"{bar} {pct:.0f}%f", inline=False)
        embed.add_field(name=self.bot.t(lang, "levels.xp_voice"), value=str(md.get("voice_xp", 0)), inline=True)
        embed.add_field(name="#⃣ " + self.bot.t(lang, "levels.rank"), value=self.bot.t(lang, "levels.rank_number", rank=rank) if rank else self.bot.t(lang, "common.na"), inline=True)
    ...
    async def leaderboard(self, interaction: discord.Interaction, tipo: str = "xp"):
        ...
        labels = {"total_xp": self.bot.t(lang, "levels.xp"), "level": self.bot.t(lang, "levels.level"), "reputation": self.bot.t(lang, "common.yes"), "balance": self.bot.t(lang, "economy.balance")}
        rows = await self.bot.db.get_leaderboard(interaction.guild.id, stat, 50)
        if not rows:
            return await send_ephemeral(interaction, embed=info_embed("🏆", self.bot.t(lang, "levels.no_leaderboard")))
        per_page = 10
        pages = []
        medals = ["🥇", "🥈", "🥉"]
        ...
        for chunk_idx, chunk in enumerate(chunks):
            embed = GuildEmbed(
                title=self.bot.t(lang, "levels.leaderboard_ranking", label=labels.get(stat, stat), guild=interaction.guild.name),
    ...
    async def xp_add(self, interaction: discord.Interaction, user: discord.Member, cantidad: int):
        ...
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.xp_added"), self.bot.t(lang, "levels.xp_added_desc", user=user.mention, amount=cantidad)))
    ...
    async def xp_remove(self, interaction: discord.Interaction, user: discord.Member, cantidad: int):
        ...
        md["xp"] = md["total_xp"] - sum(get_level_xp(l) for l in range(new_lv))
        await self.bot.db.update_member(user.id, interaction.guild.id, total_xp=md["total_xp"], level=new_lv, xp=md["xp"])
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.xp_removed"), self.bot.t(lang, "levels.xp_removed_desc", user=user.mention, amount=cantidad)))
    ...
    async def xp_set(self, interaction: discord.Interaction, user: discord.Member, cantidad: int):
        ...
        md["xp"] = md["total_xp"] - sum(get_level_xp(l) for l in range(new_lv))
        await self.bot.db.update_member(user.id, interaction.guild.id, total_xp=md["total_xp"], level=new_lv, xp=md["xp"])
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.xp_set"), self.bot.t(lang, "levels.xp_set_desc", user=user.mention, amount=cantidad)))
    ...
    async def xp_reset(self, interaction: discord.Interaction, user: discord.Member):
        ...
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.xp_reset"), self.bot.t(lang, "levels.xp_reset_desc", user=user.mention)))
    ...
    async def lr_add(self, interaction: discord.Interaction, level: int, role: discord.Role):
        ...
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.role_reward_added", level=level, role=role.mention)))
    ...
    async def lr_remove(self, interaction: discord.Interaction, level: int):
        ...
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.role_reward_removed"), self.bot.t(lang, "levels.level_role_removed", level=level)))
    ...
    async def lr_list(self, interaction: discord.Interaction):
        ...
        if not rows:
            return await send_ephemeral(interaction, embed=info_embed(self.bot.t(lang, "levels.level_roles_title"), self.bot.t(lang, "levels.no_level_roles")))
        embed = GuildEmbed(title=self.bot.t(lang, "levels.level_roles_title"), color=config.EMBED_COLOR)
        for r in rows:
            role = interaction.guild.get_role(r["role_id"])
            embed.add_field(name=f"{self.bot.t(lang, 'levels.level')} {r['level']}f", value=role.mention if role else self.bot.t(lang, "levels.level_role_deleted"), inline=False)
        await send_ephemeral(interaction, embed=embed)
    ...
    async def levelconfig(self, interaction: discord.Interaction, channel: discord.TextChannel = None, message: str = None):
        ...
        await send_ephemeral(interaction, embed=success_embed(self.bot.t(lang, "levels.level_config_updated")))
    ...
    async def levelmessage(self, interaction: discord.Interaction):
        ...
        msg = g.get("level_message", self.bot.t(lang, "levels.level_up_msg", user="{user}", level="{level}", name="{name}"))
        embed = info_embed(self.bot.t(lang, "levels.level_message_title"), self.bot.t(lang, "levels.level_message_desc", msg=msg))
        await send_ephemeral(interaction, embed=embed)
